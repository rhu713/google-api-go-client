...
// req.WithContext, then calls any functions returned by the hooks in
// reverse order.
func SendRequestWithRetry(ctx context.Context, client *http.Client, req *http.Request, retry *RetryConfig, logf func(format string, args ...interface{})) (*http.Response, error) {
	// Disallow Accept-Encoding because it interferes with the automatic gzip handling
	// done by the default http.Transport. See https://github.com/google/google-api-go-client/issues/219.
	if _, ok := req.Header["Accept-Encoding"]; ok {
		return nil, errors.New("google api: custom Accept-Encoding headers not allowed")
	...
	return sendAndRetry(ctx, client, req, retry, logf)
}

func sendAndRetry(ctx context.Context, client *http.Client, req *http.Request, retry *RetryConfig, logf func(format string, args ...interface{})) (*http.Response, error) {
	if client == nil {
		client = http.DefaultClient
	}
	...
	for {
		...
		case <-ctx.Done():
			...
			if err == nil {
				err = ctx.Err()
				if err != nil {
					logf("rh_debug: nil err ctx err: %v ", err)
				}
			}
			if err != nil {
				logf("rh_debug: not nil err: %v ", err)
			}
			return resp, err
		...
		if ctx.Err() != nil {
			...
			// canceled before.
			if err == nil {
				logf("rh_debug: nil err 2 ctx err:  %v", err)
				err = ctx.Err()
			} else {
				logf("rh_debug: not nil err 2: %v ", err)
			}
			return resp, err
		}
		...
		resp, err = client.Do(req.WithContext(ctx))
		if err != nil {
			logf("rh_debug: client.Do() err: %v ", err)
		}
		...
		if resp != nil {
			...
			if status >= 400 {
				logf("rh_debug: client.Do() response status: %d", status)
			}
		}
		...
		if req.GetBody == nil || !errorFunc(status, err) {
			logf("rh_debug: not retrying status %d nilBody %v err: %v", status, req.GetBody == nil, err)
			break
		}
		var errBody error
		req.Body, errBody = req.GetBody()
		if errBody != nil {
			logf("rh_debug: has errBody: %v", errBody)
			break
		}
		...
		logf("rh_debug: retrying err:  %v", err)
	}
	return resp, err
}
